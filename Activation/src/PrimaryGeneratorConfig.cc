...
//------------------------------------------------------------------------//
void PrimaryGeneratorConfig::SetExposureTime(G4double time)
{
  G4AutoLock lock(&fMutex);
  fTimeExposure = time;
  G4cout << "Exposure time set to " << G4BestUnit(time, "Time") << G4endl;
}

G4double PrimaryGeneratorConfig::GetExposureTime() const
{
  G4AutoLock lock(&fMutex);
  return fTimeExposure;
}
